...
module.exports = function (client, filter) {
    ...
    create: function (req, res, next) {
      ...
      async.map(req.body.msg, filter.normalise.bind(null, client, {}), function (error, results) {
        ...
        async.each(results, function (word, eachCb) {
          ...
          client.sadd([ key ].concat(results), function (error, amountAdded) {
            if (error) return next(error)

            self.emit('redsee-created:words', results, req.body.type)
  ...
  , delete: function (req, res, next) {
      ...
      async.map(req.body.msg, filter.normalise.bind(null, client, {}), function (error, results) {
        ...
        client.srem([ key ].concat(results), function (error, amountDeleted) {
          if (error) return next(error)

          if (amountDeleted) self.emit('redsee-deleted:words', results, req.body.type)
