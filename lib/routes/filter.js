...
module.exports = function (client, filter) {
  ...
  return function (req, res, next) {
    ...
    client.get(client.prefix + 'redsee-cache:filter:' + hash, function (error, cachedRes) {
      ...
      filter(client, req.body.msg, function (error, response) {
        ...
        client.set(client.prefix + 'redsee-cache:filter:' + hash, JSON.stringify(response), function (error) {
          if (error) return next(error)

          client.sadd([ client.prefix + 'redsee-cache:filter:keys' ].concat(hash), function (error) {
            if (error) return next(error)
